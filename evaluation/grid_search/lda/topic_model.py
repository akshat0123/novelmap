...
class TopicModel:
    ...
    def calc_unigram_topics(self, documents, y_axes):
    ...
    def calc_trigram_topics(self, documents, y_axes):
        """ Takes in a list of documents and a set of y axes generated from the
            'generate_y_axes' function and returns a list of average distances
            from all books to each unigram on each y axis
        """
        ...
        for y_axis_index in trange(len(y_axes), desc='Extracting Topics'):
            ...
            for doc_index in range(len(documents)):
                ...
                if y_index == 0:
                    topic = (y_axis[y_index], y_axis[y_index+1], y_axis[y_index+2])
                elif y_index == len(y_axis)-1:
                    topic = (y_axis[y_index-2], y_axis[y_index-1], y_axis[y_index])
                else:
                    topic = (y_axis[y_index-1], y_axis[y_index], y_axis[y_index+1])
